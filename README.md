# Eleventy on One9x Pages

A minimal Eleventy starter, wired to deploy to [One9x Pages](https://one9x.com)
on every push. **Use this template** to start from it, or read it as a reference
for an app you already have.

```sh
npm run build
one9x pages release ./_site --site mysite --deploy
```
...
`eleventy.config.js` sets `dir.output` to `_site`, which is also Eleventy's
default. Change it and change `dir:` in the workflow to match — the flag is just
a path.

## Docs

- [Eleventy on One9x Pages](https://one9x.com/docs/frameworks/eleventy) — the
